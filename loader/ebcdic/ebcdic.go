...
const (
	// RuneError is the `error` Rune or "Unicode replacement character", to remove
	// dependency from utf8, but check for its validity in test.
	RuneError = '\uFFFD'
)
...
// DecodeRune unpacks the first EBCIDIC encoding in p and returns the rune and
// its width in bytes. If p is empty it returns (RuneError, 0). Otherwise, if
// the encoding is invalid, it returns (RuneError, 1). Both are impossible results
// for correct, non-empty EBCIDIC.
// Note, always return (_, 1) on success.
//TODO since it always return a byte, probably you want a simpler function def
func DecodeRune(p []byte) (r rune, size int) {

	if len(p) == 0 {
		return RuneError, 0 // temp borrow const:RuneError from utf8
	}
	return rune(decodingTable[p[0]]), 1
}
